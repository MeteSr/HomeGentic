...
| # | Item | Status | Size | Notes |
|---|------|--------|------|-------|
| 8.7.1 | System-age depreciation model | ⬜ Missing | L | When a system (HVAC, roof, water heater, etc.) passes a threshold of its expected lifespan (e.g., >80% of rated life from `maintenance` canister `SYSTEM_LIFESPANS`), deduct points scaled to how far past threshold. Deduction increases as system approaches/exceeds 100% lifespan. Computed in `scoreService.ts` from `systemAgesService` data. |
| 8.7.2 | Warranty expiration score events | ⬜ Missing | M | When `warrantyExpiry` on a job fixture passes, emit a negative score event (e.g., -2 pts per expired warranty). Wire into existing `scoreEventService.ts` event pipeline. Events appear in "Score Activity" feed. Requires scheduled check: run at login and on dashboard load, compare `now` vs `warrantyExpiry` for all jobs with `warrantyMonths` set. |
| 8.7.3 | Overdue maintenance gap penalty | ⬜ Missing | M | If a system has an overdue predictive task per the `maintenance` canister (past scheduled service date), apply a rolling penalty: -1 pt per overdue critical task, capped at -5 pts total for maintenance gaps. Reset when the job is logged. Ties into `getSeasonalTasks()` / `predictMaintenance()` output — tasks with `status: Critical` past their due date qualify. |
| 8.7.4 | Inactivity decay | ⬜ Missing | S | If no new verified jobs are logged within 6 months, begin slow score decay: -1 pt/month starting month 7, capped at -6 pts total. Decay pauses when a new job is added. Implemented in `scoreService.ts` from last verified job date. Nudge notification fires at month 5 (before decay starts) — see 8.7.6. |
| 8.7.5 | Negative score event feed integration | ⬜ Missing | S | Decay events appear in the existing "Score Activity" feed (Dashboard) with red/negative styling and plain-English explanations: "HVAC warranty expired — -2 pts", "Roof is 22 years old (rated life: 20 yrs) — -3 pts". Extend `ScoreEvent` type in `scoreEventService.ts` with `delta: negative` variant and `decayCategory` field. |
| 8.7.6 | Score recovery action prompts | ⬜ Missing | M | When a decay event fires, generate a paired recovery prompt shown on Dashboard and in the Score Activity feed: "Your HVAC is overdue for service — log a tune-up to recover 3 pts." Prompts link to `JobCreatePage` or `PredictiveMaintenancePage`. Implemented as a `DecayPrompt[]` derived list in `scoreService.ts` alongside `ScoreEvent[]`. |
| 8.7.7 | "Score at Risk" dashboard warning | ⬜ Missing | S | When pending decay events exist within the next 30 days (warranty expiring, system crossing age threshold), show a dismissible "Score at Risk" warning card on Dashboard above the score arc. Lists the specific upcoming hits with dates and point values. Motivates proactive maintenance logging before the penalty lands. |
| 8.7.8 | Decay floor — minimum score guarantee | ⬜ Missing | S | No matter how old or neglected, decay cannot push the score below a floor (e.g., 30). A home with a long verified history can't fall to zero. The floor is configurable and reflects that the on-chain record itself has value independent of current condition. Prevents extreme edge cases from making the score useless as a signal. |
