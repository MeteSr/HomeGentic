...
Covered so far: landing page, pricing page, dashboard, property detail, jobs, contractors, market intelligence, and the AddPropertyModal onboarding wizard (address / details / saved-hub steps) — each at the desktop (1280×800), mobile (375×812), and tablet (768×1024) projects. A failure over 0.1% pixel diff (`maxDiffPixelRatio` in the config) fails the run; diffs are uploaded as a `visual-diff-report` artifact on CI failure.
