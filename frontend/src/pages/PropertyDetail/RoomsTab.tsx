      ...
      >
        ...
        <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 12 }}>
          <div>
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              <span style={{ fontFamily: DISPLAY, fontSize: 17, fontWeight: 800, color: "var(--hg-ink)" }}>
                {room.name}
              </span>
              {receipted > 0 && (
                <span style={{ fontFamily: MONO, fontSize: 9, fontWeight: 700, color: "var(--hg-blue-ink)", background: "var(--hg-blue-wash)", border: "1px solid var(--hg-blue-edge)", borderRadius: 4, padding: "2px 7px" }}>
                  {receipted} RECEIPTED
                </span>
              )}
            </div>
          ...
          <button
            ...
            style={{ background: "none", border: "1px solid var(--hg-line-2)", borderRadius: 6, padding: "4px 10px", fontFamily: BODY, fontSize: 12, color: "var(--hg-muted)", cursor: "pointer" }}
          >
            ›
          </button>
        </div>
        ...
        {room.floorType && (
          <div style={{ marginBottom: 10 }}>
            <div style={{ fontFamily: MONO, fontSize: 9, fontWeight: 700, color: "var(--hg-muted)", letterSpacing: "0.1em", textTransform: "uppercase", marginBottom: 4 }}>
              FLOORING
            </div>
            <div style={{ fontFamily: BODY, fontSize: 13, fontWeight: 600, color: "var(--hg-ink)" }}>
              {room.floorName || room.floorType}
            </div>
            {room.paintCode && (
              <div style={{ fontFamily: BODY, fontSize: 12, color: "var(--hg-blue-ink)" }}>
                {room.paintCode}
              </div>
            )}
          </div>
        ...
        {/* Footer */}
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginTop: 14, paddingTop: 12, borderTop: "1px solid var(--hg-line)" }}>
          <span style={{ fontFamily: BODY, fontSize: 12, color: "var(--hg-muted)" }}>
            {finishCount} finish{finishCount !== 1 ? "es" : ""} recorded
          </span>
          <div style={{ display: "flex", gap: 8 }}>
            <button
              onClick={() => onAddFixture(room.id)}
              style={{ fontFamily: BODY, fontSize: 12, fontWeight: 500, color: "var(--hg-blue-ink)", background: "none", border: "none", padding: 0, cursor: "pointer" }}
            >
              + Add
            </button>
            <button
              onClick={() => { if (window.confirm(`Delete "${room.name}"?`)) onDelete(room.id); }}
              style={{ fontFamily: BODY, fontSize: 12, color: "var(--hg-muted)", background: "none", border: "none", padding: 0, cursor: "pointer" }}
            >
              Delete
            </button>
          </div>
...
function EditRoomDrawer({
  ...
  const inp: React.CSSProperties = { ...hudInputStyle, width: "100%", boxSizing: "border-box" };
  const lbl: React.CSSProperties = {
    display: "block", fontFamily: MONO, fontSize: 9, fontWeight: 700,
    letterSpacing: "0.1em", textTransform: "uppercase", color: "var(--hg-muted)", marginBottom: 4,
  };
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.6)", zIndex: 500, display: "flex", alignItems: "flex-end", justifyContent: "center" }} onClick={onClose}>
      <div style={{ background: "var(--hg-surface)", border: "1px solid var(--hg-line)", borderBottom: "none", borderRadius: "16px 16px 0 0", padding: 24, width: "100%", maxWidth: 540, maxHeight: "80vh", overflowY: "auto" }} onClick={e => e.stopPropagation()}>
        <h3 style={{ fontFamily: DISPLAY, fontSize: 18, fontWeight: 800, color: "var(--hg-ink)", marginBottom: 20 }}>Edit {room.name}</h3>
        ...
        <div style={{ display: "flex", gap: 10, marginTop: 20 }}>
          <button onClick={handleSave} disabled={saving || !form.name.trim()} style={{ flex: 1, fontFamily: BODY, fontSize: 14, fontWeight: 700, color: "#FCFCFD", background: "var(--hg-blue)", border: "none", borderRadius: 100, padding: "11px", cursor: "pointer", opacity: saving || !form.name.trim() ? 0.6 : 1 }}>
            {saving ? "Saving…" : "Save changes"}
          </button>
          <button onClick={onClose} style={{ fontFamily: BODY, fontSize: 14, color: "var(--hg-muted)", background: "none", border: "1px solid var(--hg-line-2)", borderRadius: 100, padding: "11px 20px", cursor: "pointer" }}>
            Cancel
          </button>
        </div>
      </div>
...
function AddFixtureDrawer({ roomId, onSave, onClose }: { roomId: string; onSave: (id: string, args: AddFixtureArgs) => Promise<void>; onClose: () => void }) {
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.6)", zIndex: 500, display: "flex", alignItems: "flex-end", justifyContent: "center" }} onClick={onClose}>
      <div style={{ background: "var(--hg-surface)", border: "1px solid var(--hg-line)", borderBottom: "none", borderRadius: "16px 16px 0 0", padding: 24, width: "100%", maxWidth: 540, maxHeight: "80vh", overflowY: "auto" }} onClick={e => e.stopPropagation()}>
        <h3 style={{ fontFamily: DISPLAY, fontSize: 18, fontWeight: 800, color: "var(--hg-ink)", marginBottom: 20 }}>Add Appliance / Fixture</h3>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
          <div>
        ...
        <div style={{ display: "flex", gap: 10, marginTop: 20 }}>
          <button onClick={handleSave} disabled={saving} style={{ flex: 1, fontFamily: BODY, fontSize: 14, fontWeight: 700, color: "#FCFCFD", background: "var(--hg-blue)", border: "none", borderRadius: 100, padding: "11px", cursor: "pointer", opacity: saving ? 0.6 : 1 }}>
...
export function RoomsTab({
  ...
  return (
    <div>
      ...
      {/* Stats row */}
      {rooms.length > 0 && (
        <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 1, background: "var(--hg-line)", border: "1px solid var(--hg-line)", borderRadius: 10, overflow: "hidden", marginBottom: 24 }}>
          {[
            ...
            <div key={label} style={{ background: "var(--hg-surface)", padding: "14px 16px" }}>
              <div style={{ fontFamily: MONO, fontSize: 9, fontWeight: 700, color: "var(--hg-muted)", letterSpacing: "0.1em", textTransform: "uppercase", marginBottom: 4 }}>
                {label}
              </div>
              <div style={{ fontFamily: DISPLAY, fontSize: 20, fontWeight: 900, color: "var(--hg-ink)" }}>
                {value}
              </div>
            </div>
          ))}
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16 }}>
        <p style={{ fontFamily: BODY, fontSize: 14, color: "var(--hg-muted)" }}>
        ...
        <button
          ...
          style={{ display: "flex", alignItems: "center", gap: 6, fontFamily: BODY, fontSize: 14, fontWeight: 600, color: "#FCFCFD", background: "var(--hg-blue)", border: "none", borderRadius: 100, padding: "10px 18px", cursor: "pointer", flexShrink: 0, marginLeft: 16 }}
        >
          + Add room
        </button>
      </div>
      ...
      {rooms.length === 0 ? (
        <div style={{ textAlign: "center", padding: "48px 24px", border: "1px dashed var(--hg-line-2)", borderRadius: 12 }}>
          <p style={{ fontFamily: BODY, fontSize: 15, fontWeight: 600, color: "var(--hg-ink)", marginBottom: 6 }}>No rooms yet</p>
          ...
          <button onClick={() => setShowAddRoom(true)} style={{ fontFamily: BODY, fontSize: 14, fontWeight: 600, color: "#FCFCFD", background: "var(--hg-blue)", border: "none", borderRadius: 100, padding: "10px 24px", cursor: "pointer" }}>
            + Add your first room
          </button>
        </div>
      ) : (
