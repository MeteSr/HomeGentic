  ...
  return (
    <div onClick={onExpand} style={{ cursor: onExpand ? "pointer" : "default", marginTop: "0.5rem" }}>
      <svg width={W} height={H} style={{ display: "block", opacity: 0.7 }}>
        <polyline points={pts} fill="none" stroke={V2_COLORS.cobalTint} strokeWidth="1.5" strokeLinejoin="round" />
        {scores.map((s, i) => {
          const x = pad + (i / (scores.length - 1)) * (W - pad * 2);
          const y = H - pad - ((s - min) / range) * (H - pad * 2);
          return i === scores.length - 1
            ? <circle key={i} cx={x} cy={y} r="2.5" fill={V2_COLORS.blue} />
            : null;
        })}
      </svg>
      {onExpand && (
        <div style={{ fontFamily: V2_FONTS.body, fontSize: "0.5rem", letterSpacing: "0.08em", textTransform: "uppercase", color: V2_COLORS.muted, marginTop: "0.25rem" }}>
          View history ↗
        </div>
      )}
    </div>
